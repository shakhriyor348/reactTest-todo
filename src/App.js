  ...
  return (
    <div className="todo">
      <div className="todo__sidebar">
        ...
        <List items={lists}
          isRemovable
          onRemove={(item) => setLists(lists.filter(list => list.id !== item.id))}
          onClickItem={item => {
      ...
      <div className="todo__tasks">
        <Routes>
          <Route exact path="/" element={lists.map(list => (
            <Tasks key={list.id} onAddTask={onAddTask} list={list} onEditTitle={onEditListTitle} withoutEmpty onRemoveTask={onRemoveTask} onEditTaskText={onEditTaskText} onCompleteTask={onCompleteTask}/>
          )
          )} />
        </Routes>

        {lists && activeItem && <Tasks onAddTask={onAddTask} list={activeItem} onEditTitle={onEditListTitle} onRemoveTask={onRemoveTask} onEditTaskText={onEditTaskText} onCompleteTask={onCompleteTask}/>}
      </div>
    </div>
  );
}
